defmodule Seed.Accounts.AuthorToken do
  use Ecto.Schema
  import Ecto.Query
  ...
  schema "authors_tokens" do
    ...
    belongs_to :author, Seed.Accounts.Author
  ...
  def build_session_token(author) do
    ...
    {token, %Seed.Accounts.AuthorToken{token: token, context: "session", author_id: author.id}}
  end
  ...
  def token_and_context_query(token, context) do
    from Seed.Accounts.AuthorToken, where: [token: ^token, context: ^context]
  end
  ...
  def author_and_contexts_query(author, :all) do
    from t in Seed.Accounts.AuthorToken, where: t.author_id == ^author.id
  end

  def author_and_contexts_query(author, [_ | _] = contexts) do
    from t in Seed.Accounts.AuthorToken, where: t.author_id == ^author.id and t.context in ^contexts
  end
end
